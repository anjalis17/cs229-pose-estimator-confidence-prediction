# src/analysis/plot_iw_weights.py
"""
Beat 5 — why importance weighting is inert here (free insurance, not a fix).

Histogram of the synthetic importance weights w = p/(1-p) toward each HIL domain,
where p = P(HIL | x) from the domain classifier. A spike near w ≈ 0 with a thin
tail means most synthetic images look nothing like the target domain, so
reweighting concentrates on a few samples (low effective sample size) and barely
changes the failure classifier — consistent with IW ≈ supervised in the ROC/AUC.

Reuses the domain classifier + weighting from src/pipeline/models.py.

Saves:
    figures/iw_weights.{png,pdf}
"""
